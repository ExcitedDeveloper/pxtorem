  ...
  return (
    <div>
      ...
      <button 
        ...
        onClick={() => setCurrentTheme(themeType === ThemeType.Light ? ThemeType.Dark : ThemeType.Light)}
      >
        Toggle Theme
      </button>
    </div>
...
describe('ThemeProvider', () => {
  ...
  it('should provide default light theme when no stored theme', () => {
    ...
    expect(screen.getByTestId('primary-color')).toHaveTextContent(THEMES.light['--primary'])
  })
  ...
  it('should load stored theme from localStorage', () => {
    ...
    expect(screen.getByTestId('primary-color')).toHaveTextContent(THEMES.dark['--primary'])
  })
  ...
  it('should save theme to localStorage when theme changes', async () => {
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith('currentTheme', JSON.stringify(ThemeType.Dark))
  })
