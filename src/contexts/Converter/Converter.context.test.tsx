...
import { ConverterProvider, useConverter, ConversionDirection } from './Converter.context'

// Test component to use the converter context
const TestComponent = () => {
  const { direction, setDirection, pixels, setPixels, rootFontSize, setRootFontSize } = useConverter()
  ...
  return (
    <div>
      ...
      <button 
        ...
        onClick={() => setDirection(direction === ConversionDirection.PxToRem ? ConversionDirection.RemToPx : ConversionDirection.PxToRem)}
      >
        Toggle Direction
      </button>
      <button 
        data-testid="set-pixels" 
        onClick={() => setPixels(24)}
      >
        Set Pixels
      </button>
      <button 
        data-testid="set-font-size" 
        onClick={() => setRootFontSize(18)}
      >
        Set Font Size
      </button>
    </div>
...
describe('ConverterProvider', () => {
  ...
  it('should provide default values', () => {
    ...
    expect(screen.getByTestId('direction')).toHaveTextContent(ConversionDirection.PxToRem.toString())
  ...
  it('should update direction when setDirection is called', async () => {
    ...
    expect(screen.getByTestId('direction')).toHaveTextContent(ConversionDirection.RemToPx.toString())
  })
  ...
  it('should update root font size and save to localStorage', async () => {
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith('root-font-size', JSON.stringify(18))
  })
